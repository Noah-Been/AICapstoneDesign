...
def parse_eugene_daily_rows(html: str, target_date: str) -> List[Dict[str, Any]]:
    soup = BeautifulSoup(html, "lxml")
    out: List[Dict[str, Any]] = []
    seen: set[Tuple[str, str]] = set()
    target_date_slash = target_date.replace('-', '/')

    # Universal parser for both list styles
    for li in soup.select("ul#list > li, ul.iss_list > li"):
        a_tag = li.find("a")
        ...
        date_txt = (date_el.get_text(strip=True) or "").replace('.', '/')
        if date_txt != target_date_slash: continue
        ...
        author_el = li.find(class_="writer") or li.find(class_="name")
        if author_el:
            author = author_el.get_text(strip=True)

        if not title:
            title_strong = a_tag.find("strong")
            if title_strong: title = title_strong.get_text(strip=True)

        pdf_url = urljoin(BASE, a_tag["href"])
        detail_url = pdf_url # Since it links directly to PDF

        key = (title, canon_url(pdf_url))
        ...
        out.append({
            ...
            "house_id": "eugene",
            "title": title,
            "url": detail_url, # Use PDF url as the main url
            "pdf_url": pdf_url, # Add pdf_url field
...
# --- Stage 2: Download, OCR, Refine (Reused from Mirae Pipeline) ---

def download_pdf(session: httpx.Client, pdf_url: str, temp_dir: str) -> str | None:
    try:
        print(f"  [Step 1b] Downloading PDF from {pdf_url}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Full pipeline for Eugene Investment reports: Crawl -> Download -> OCR -> Refine -> Save")
    ...
    print(f"--- Starting Eugene Investment Pipeline for {args.date} ---")
    with httpx.Client(headers=headers, timeout=20.0, follow_redirects=True, verify=False) as session:
        reports_to_process = []
        list_paths = ["/ii30r.do", "/ii31r.do"] # Corporate and Industry analysis
        print(f"--- Fetching report lists from {list_paths} ---")
        for path in list_paths:
            try:
                print(f"Fetching list from {BASE}{path}...")
                list_html = session.get(f"{BASE}{path}").text
                reports_to_process.extend(parse_eugene_daily_rows(list_html, args.date))
            except Exception as e:
                print(f"Failed to fetch or parse {path}: {e}")

        # Deduplicate after collecting from all sources
        deduped_reports = []
        seen_keys = set()
        for report in reports_to_process:
            key = (report['title'], report['url'])
            if key not in seen_keys:
                seen_keys.add(key)
                deduped_reports.append(report)
        
        reports_to_process = deduped_reports
        print(f"Found {len(reports_to_process)} unique reports to process.")
        ...
        for report in reports_to_process:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    eugene_jsonl_path = os.path.join(output_dir_dated, "by_house", "eugene.jsonl")
                    os.makedirs(os.path.dirname(eugene_jsonl_path), exist_ok=True)
                    
                    append_to_jsonl(all_jsonl_path, final_data)
                    append_to_jsonl(eugene_jsonl_path, final_data)
